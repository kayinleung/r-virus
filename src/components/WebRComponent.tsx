...
export const WebRComponent = () => {
  ...
  useEffect(() => {
    ...
    const compute = async () => {
      const rCode = currentForm.value.modelType === 'model_reference' ?
        rCodeModelReference :
        (currentForm.value.modelType === 'model_network' ?
          rCodeModelNetwork :
          rCodeModelNetworkNB
        );
      ...
      readWebRDataElementsEvents(webR, currentForm.value.modelType);
    };
    compute();
